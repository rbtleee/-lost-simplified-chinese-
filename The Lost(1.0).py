...
class Player:
    ...
    def check_pet_hunger(self):
        ...
        return len(hungry_pets) == 0  # 返回True表示没有饥饿宠物
    ...
    def upgrade_equipment(self):
        ...
        if self.inventory['兽皮'] >= 5 and self.inventory['纤维'] >= 3:
            options.append(('升级护甲', '消耗5兽皮和3纤维提升护甲等级'))
    ...
    def check_evolution(self):
        if len(self.active_pets) >= 3:  # 改为>=3更合理
        # 检查是否有3只同类型宠物
            ...
            for pet_type, count in type_counts.items():
                if count >= 3:  # 如果有3只或更多同类型
                # 获取所有同类型宠物并按等级排序（方便选择保留哪只）
                    same_type_pets = [p for p in self.active_pets if p.type == pet_type]
                    same_type_pets.sort(key=lambda x: x.level, reverse=True)  # 按等级降序
                ...
                # 创建进化后的新宠物
                    evolved_pet = copy.deepcopy(same_type_pets[0])  # 复制最高级的宠物
                    evolved_pet.level = new_level
                    evolved_pet.evolution += 1  # 假设宠物类有evolution属性
...
def beach_event(player):
    ...
    # 提高钥匙获取概率到40%
    if random.random() < 0.6:  # 40%几率找到钥匙
        player.inventory['钥匙'] += 1
        print("找到1把钥匙！")
    else:
        found = random.choice([
...
def beast_event(player, location):
    ...
    if player.active_pets:
        ...
        player_power += len(player.active_pets) * 2 + total_stars * 1# 基础加成：每只宠物3点攻击力，每星级额外2点
    ...
    while player.health > 0 and beast_hp > 0:
        ...
        if escape_choice == 'y':
            escape_chance = 0.6  # 基础逃跑成功率60%
            if "闪避" in player.monster_skills:
                escape_chance += player.monster_skills["闪避"] * 0.05  # 每级闪避技能增加5%逃跑成功率
    ...
    # 怪物技能加成
        for skill, level in player.monster_skills.items():
            if skill == "黑虎掏心" :
                ...
                beast_hp -= beast_hp * 4 // (beast_level * 20 + 150) * level
                print(f"使用技能毒杀，对怪物造成{beast_hp*4//(beast_level*20+150)*level}点附加伤害")
            elif skill == "撕咬" :
                beast_hp -= 2 * level
        ...
        # 解锁或升级怪物技能
            if random.random() < 0.2 and beast_type in skill_options:
...
def text_adventure():
    ...
    while not game_over:
        ...
        if player.health_points <= 0:
            print("\n你的健康状况恶化，无法继续冒险！")
            print("游戏结束！看广告复活牢大")
            reborn = input("是否观看30s广告领取奖励？(y/n): ").lower()
            if reborn == 'y':
                print("牢大复活了，但是你死了")
            break
