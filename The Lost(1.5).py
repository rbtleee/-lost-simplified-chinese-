...
class Player:
    ...
    def level_up(self):
        ...
        try:
            ...
            if 0 <= choice < len(skill_options):
                ...
                if self.passive_skills.get(selected_skill, 0) >= 20:
                    print(f"{selected_skill}已达到最高等级(20级)，改为获得随机物品")
    ...
    def get_random_item(self):
        items = ['石头', '木头', '纤维', '钥匙', '盐', '生肉', '熏肉', '鱼', '熏鱼', '兽皮', '水晶', '草药', '药膏']
        item = random.choice(items)
        amount = random.randint(1, 5)
        self.inventory[item] += amount
        print(f"获得{item}x{amount}")
    ...
    def manage_pets(self):
        ...
        while True:
            ...
            for i, pet in enumerate(self.pets, 1):
                ...
                print(f"{i}. {pet.name} ({pet.type}, Lv{pet.level}, 星级: {pet.evolution}, 状态: {hunger_status})")
        
            print("\n当前携带的宠物:")
            for i, pet in enumerate(self.active_pets, 1):
                print(f"{i}. {pet.name} ({pet.type}, 星级: {pet.evolution})") 
    ...
    def evolve_pet(self):
        """手动升星宠物"""
        if len(self.pets) < 3:
            print("至少需要3只宠物才能进行升星操作！")
            return
    
        print("\n选择3只同类型宠物进行升星（星级会继承最高值+1）")
        print("可用宠物列表:")
    
    # 按类型分组显示宠物
        pets_by_type = defaultdict(list)
        for i, pet in enumerate(self.pets, 1):
            pets_by_type[pet.type].append((i, pet))
    
        for pet_type, pets in pets_by_type.items():
            print(f"\n{pet_type}:")
            for idx, pet in pets:
                print(f"  {idx}. {pet.name} (Lv{pet.level}, 星级: {pet.evolution})")
    
        try:
        # 选择3只宠物
            selected = input("输入3只宠物的编号，用空格分隔（例如：2 5 7）: ").split()
            if len(selected) != 3:
                print("必须选择3只宠物！")
                return
            
            selected = [int(idx)-1 for idx in selected]  # 转为0-based索引
            selected_pets = [self.pets[idx] for idx in selected]
        
        # 检查是否同类型
            pet_type = selected_pets[0].type
            if not all(pet.type == pet_type for pet in selected_pets):
                print("必须选择同类型的宠物！")
                return
        
        # 计算新星级和等级
            max_evolution = max(p.evolution for p in selected_pets)
            total_level = sum(p.level for p in selected_pets)
            new_level = total_level // 2  # 新等级取平均
        
        # 创建新宠物
            new_pet = copy.deepcopy(selected_pets[0])
            new_pet.evolution = max_evolution + 1
            new_pet.level = new_level
        
        # 命名
            new_name = input(f"给新的{pet_type}起个名字（留空使用默认名）: ")
            if new_name:
        ...
        # 移除旧宠物，添加新宠物
            ...
            print(f"\n升星成功！获得 {new_pet.name} (Lv{new_pet.level}, ★{new_pet.evolution})")
            ...
            if luck < 0.1:
                ...
                try:
                    ...
                    if 0 <= cave_skill_number < len(skill_options):
                        ...
                        if player.monster_skills.get(cave_skill, 0) >= 20:
                            print(f"{cave_skill}已达到最高等级(20级)，改为获得随机物品")
                            player.get_random_item()
                        else:
...
def beast_event(player, location):
    beast_types = {
        ...
        "山脉": ["熊", "鹰", "雪豹"],
        "湖泊": ["鳄鱼", "水蛇", "巨型青蛙"],
        "遗迹": ["守卫者", "远古石像", "亡灵祭司", "变异巨兽"]  # 修复了字符串拼接问题
    }
        ...
        if beast_hp <= 0 and player.health > 0:
            ...
            if beast_type in skill_options:
                ...
                if random.random() < base_chance:
                    if player.monster_skills.get(skill, 0) >= 20:
                        print(f"{skill}已达到最高等级(20级)，改为获得随机物品")
                        player.get_random_item()
                    else:
            ...
            if menu_choice == 1:
                ...
                try:
                    ...
                    if gift_type == 1:
                        ...
                        # 升级被动技能
                        if player.passive_skills:
                            for skill in list(player.passive_skills.keys()):
                                if player.passive_skills[skill] >= 20:
                                    print(f"{skill}已达到最高等级(20级)，改为获得随机物品")
                                    player.get_random_item()
                                else:
                        ...
                        # 升级怪物技能
                        if player.monster_skills:
                            for skill in list(player.monster_skills.keys()):
                                if player.monster_skills[skill] >= 20:
                                    print(f"{skill}已达到最高等级(20级)，改为获得随机物品")
                                    player.get_random_item()
                                else:
                                    player.monster_skills[skill] += 1
